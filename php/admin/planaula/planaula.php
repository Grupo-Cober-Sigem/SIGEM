<?php
    ...
    switch($opcion){
        ...
        case 2: //Modificar
            ...
            break;
        case 3: //Filtrar
            $consulta = "SELECT Asignatura.Nombre_Asig, Asignatura.Cod_Asignatura, Asignatura.Ubicacion_Asig FROM Asignatura INNER JOIN planAula WHERE Asignatura.Cod_Asignatura = planAula.Cod_Asignatura AND Asignatura.Cod_Asignatura LIKE CONCAT('%','$codigo','%')";
            $resultado = $cn->prepare($consulta);
            $resultado->execute();
            $data = $resultado->fetchAll(PDO::FETCH_ASSOC);
            break;
